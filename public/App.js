...
const Home = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	do {
		...
		$$rendered = `<h1>Architectuur van Indonesië</h1>
		<ul>
			${each(data, (source, i) => `<a href="details/${escape(source.cho.value.slice(-6))}"><li>
				<h2>${escape(source.modelNaam.value)}</h2>
				<img${add_attribute("src", source.imageModel.value, 0)}${add_attribute("alt", source.modelNaam.value, 0)} class="svelte-h48pex">
				${validate_component(((source[i]) || missing_component), 'svelte:component').$$render($$result, { this: instances[i] }, {
			this: $$value => { instances[i] = $$value; $$settled = false; }
		}, {})}
			</li></a>`)}
		</ul>`;
	} while (!$$settled);
...
const Detail = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	if ($$props.queryUrl === void 0 && $$bindings.queryUrl && queryUrl !== void 0) $$bindings.queryUrl(queryUrl);

	return `<h1>tijdelijk</h1>`;
});
